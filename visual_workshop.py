...
class CollapsibleGroupBox(QGroupBox):
    ...
    def _toggle_content_internal(self, checked):
        ...
        if checked:
            self.content_widget.setSizePolicy(QSizePolicy.Policy.Preferred, QSizePolicy.Policy.Expanding) # Allow expansion
...
class VisualWorkshopApp(QMainWindow):
    ...
    # --- Basic Config UI Methods (largely unchanged, but simplified where needed) ---
    ...
    def _add_bar_config(self, layout):
        ...
        self.bar_mode_combo.addItems(["Dodge", "Stack"]) # Matplotlib/Seaborn equivalent terms
    ...
    def _add_histogram_config(self, layout):
        ...
        norm_combo.addItems(["Count", "Frequency", "Probability", "Density"]) # Seaborn options
    ...
    # --- Data Handling Methods (largely unchanged) ---
    ...
    def generate_plot(self):
        ...
        if self.sample_random_rb.isChecked():
            ...
            target_col_for_sampling = None # Determine target column based on plot type
        ...
        try:
            self.fig.clear()
            pd_df = df_to_plot.to_pandas() # Convert to pandas for seaborn/matplotlib
            ...
            if use_facets:
                # Faceting is complex to manage manually. For this rewrite, we'll use Seaborn's FacetGrid for simplicity if applicable,
                # or show a message that manual faceting is needed. Given the UI, manual is better.
    ...
    def _draw_plot_on_ax(self, ax, plot_type, df_pd, basic_args, advanced_args):
        # Router to different plotting functions
        plot_drawers = {
            "Scatter Plot": self._draw_scatter, "Line Plot": self._draw_line,
            "Bar Chart": self._draw_bar, "Pie Chart": self._draw_pie,
            "Histogram": self._draw_histogram, "Box Plot": self._draw_box,
            "Violin Plot": self._draw_violin, "Strip Plot": self._draw_strip,
            "Density Contour": self._draw_density, "Distribution Plot (Distplot)": self._draw_distplot,
            "Time Series Plot": self._draw_timeseries,
        }
    ...
    def save_current_plot(self):
        ...
        file_name, selected_filter = QFileDialog.getSaveFileName(self, "Save Plot As", default_filename,
    ...
    # --- INDIVIDUAL PLOT DRAWING METHODS (using Matplotlib/Seaborn) ---

    def _get_seaborn_kwargs(self, basic_args, advanced_args):
        """Helper to get common seaborn arguments like hue, style, size, palette."""
        kwargs = {}
        if basic_args.get('color_col') != "None": kwargs['hue'] = basic_args['color_col']
        if basic_args.get('symbol_col') != "None": kwargs['style'] = basic_args['symbol_col']
        if basic_args.get('size_col') != "None": kwargs['size'] = basic_args['size_col']
        if advanced_args.get('colormap_edit'): kwargs['palette'] = advanced_args['colormap_edit']
        return kwargs
    
    def _draw_scatter(self, ax, df, basic, adv):
        sns.scatterplot(data=df, x=basic['x_col'], y=basic['y_col'], ax=ax, **self._get_seaborn_kwargs(basic, adv))

    def _draw_line(self, ax, df, basic, adv):
        df_plot = df.sort_values(by=basic['x_col']) if basic.get('sort_x', True) else df
        sns.lineplot(data=df_plot, x=basic['x_col'], y=basic['y_col'], ax=ax, marker='o', **self._get_seaborn_kwargs(basic, adv))

    def _draw_bar(self, ax, df, basic, adv):
        # Bar chart requires aggregation first, which was done in polars
        # The logic here assumes data is already aggregated if needed.
        # For simplicity, using seaborn's barplot which can do estimation.
        orient = 'v' if basic.get('orientation') == 'Vertical' else 'h'
        x, y = (basic['x_col'], basic['y_col']) if orient == 'v' else (basic['y_col'], basic['x_col'])
        
        agg_map = {'mean': np.mean, 'sum': np.sum, 'median': np.median} # Seaborn estimator mapping
        estimator = agg_map.get(basic['agg_func'], 'mean') if basic['y_col'] != 'None' else 'count'

        sns.barplot(data=df, x=x, y=y, ax=ax,
                    orient=orient,
                    hue=basic.get('color_col') if basic.get('color_col') != 'None' else None,
                    palette=adv.get('colormap_edit') or None,
                    estimator=estimator)

    def _draw_pie(self, ax, df, basic, adv):
        names_col, mode = basic['names_col'], basic['pie_mode']
        if mode == "Count Occurrences (Rows)":
            data = df[names_col].value_counts()
        else:
            values_col = basic['values_col']
        ...
        if adv.get('explode_edit'):
            try: explode = [float(x.strip()) for x in adv['explode_edit'].split(',')]
    ...
    def _draw_histogram(self, ax, df, basic, adv):
        sns.histplot(data=df, x=basic['x_col'], 
                     weights=basic.get('y_col') if basic.get('y_col') != 'None' else None,
                     hue=basic.get('color_col') if basic.get('color_col') != 'None' else None,
                     bins=basic['nbinsx'] or 'auto',
    ...
    def _draw_violin(self, ax, df, basic, adv):
        ...
        if basic.get('split_by_col') != 'None':
            hue = basic['split_by_col']
            if len(df[hue].unique()) == 2:
                split = True

        sns.violinplot(data=df, x=x, y=y, hue=hue, split=split,
                       orient=orient, inner='box' if basic.get('box_visible') else 'quartile', ax=ax,
                       palette=adv.get('colormap_edit') or None)
    ...
    def _draw_density(self, ax, df, basic, adv):
        sns.kdeplot(data=df, x=basic['x_col'], y=basic['y_col'],
                    hue=basic.get('color_col') if basic.get('color_col') != 'None' else None,
    ...
    def _draw_distplot(self, ax, df, basic, adv):
        ...
        if not cols_to_plot:
            ax.text(0.5, 0.5, "No columns selected.", ha='center', va='center')
            return
        ...
        for col in cols_to_plot:
            sns.kdeplot(data=df, x=col, ax=ax, 
                        ...
                        cut=0,
                        cumulative=False, # Complex to UI, omitted for now
                        rug=basic.get('show_rug', False))
        if len(cols_to_plot) > 1: ax.legend()

    def _draw_timeseries(self, ax, df, basic, adv):
        x_col, y_col = basic['x_col'], basic['y_col']
        df_plot = df.sort_values(by=x_col) if basic.get('sort_x', True) else df

        # 1. Budget Bars
        if basic.get('budget_col') != "None":
            ax.bar(df_plot[x_col], df_plot[basic['budget_col']], label='Budgeted', color='sandybrown', alpha=0.6)

        # 2. Confidence Interval
        if basic.get('forecast_lower_col') != "None" and basic.get('forecast_upper_col') != "None":
            ax.fill_between(df_plot[x_col], df_plot[basic['forecast_lower_col']], df_plot[basic['forecast_upper_col']],
                            color='gray', alpha=0.3, label='Forecast CI')

        # 3. Forecast Line
        if basic.get('forecast_col') != "None":
            ax.plot(df_plot[x_col], df_plot[basic['forecast_col']], 'k--', label='Forecast')
        ...
        if basic.get('add_trendline'):
            ...
            sns.regplot(data=df_plot, x=x_col, y=y_col, ax=ax, scatter=False, color='purple',
                        line_kws={'linestyle':'--'}, label='Trendline')
        
        # 6. Actuals Line (on top)
        ax.plot(df_plot[x_col], df_plot[y_col], 'o-', color='royalblue', label='Actual')
        
        # 7. Secondary Y-Axis
        if basic.get('y2_col') != "None":
            ax2 = ax.twinx()
            ax2.plot(df_plot[x_col], df_plot[basic['y2_col']], '.-', color='firebrick', label=basic['y2_col'])
            ax2.set_ylabel(basic['y2_col'])
            ax2.legend(loc='upper right')

        ax.legend(loc='upper left')

    def _apply_matplotlib_layout(self, fig, ax, advanced_args, basic_args, plot_type):
        """Applies advanced layout settings to a Matplotlib figure and axes."""
        if ax is None: return

        # For faceted plots, ax is an array. We apply settings to each.
        ...
        # Labels (apply to first axis in a facet grid, or the only axis)
        main_ax = axs[0]
        ...
        if not legend_title:
            if basic_args.get('color_col') != 'None': 
                legend_title = basic_args['color_col']
            elif basic_args.get('symbol_col') != 'None': 
                legend_title = basic_args['symbol_col']
        ...
        for current_ax in axs:
            ...
            try:
                if advanced_args.get('xaxis_range_edit'):
                    min_val, max_val = map(float, advanced_args['xaxis_range_edit'].split(','))
                    current_ax.set_xlim(min_val, max_val)
                if advanced_args.get('yaxis_range_edit'):
                    min_val, max_val = map(float, advanced_args['yaxis_range_edit'].split(','))
    ...
    def closeEvent(self, event):
        plt.close(self.fig) # Clean up matplotlib figure
...
if __name__ == '__main__':
    ...
    class DummyPickaxe:
        def get_current_dataframe_for_vw(self):
            ...
            df = pl.from_pandas(pd.DataFrame(data)) # Create from pandas for datetime consistency
